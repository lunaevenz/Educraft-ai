...
try {
  samples.forEach(s => {
    const sql = `INSERT OR IGNORE INTO worksheets (id, title, grade, subject, skill, difficulty, theme, learning_style, instructions, content, answer_key) 
                 VALUES ('${s.id}', '${s.title}', '${s.grade}', '${s.subject}', '${s.skill}', '${s.difficulty}', '${s.theme}', '${s.learningStyle}', '${s.instructions}', '${s.content.replace(/'/g, "''")}', '${s.answer_key.replace(/'/g, "''")}')`;
    // Escape for shell
